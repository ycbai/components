 ...
 *
 * <p>
 * This interface is deliberately distinct from {@link java.util.Iterator} because the current model tends to be easier
 * to program and more efficient in practice for iterating over sources such as files, databases etc. (rather than pure
 * collections).
 *
 * <p>
 ...
 * </ul>
 * <li>Repeatedly, a call to {@link #advance}. This may be called regardless of what the previous {@link #start}/
 * {@link #advance} returned.
 * <ul>
 ...
 *
 * <p>
 * All {@code Reader} functions except {@link #getCurrentSource} do not need to be thread-safe; they may only be
 * accessed by a single thread at once. However, {@link #getCurrentSource} needs to be thread-safe, and other functions
 * should assume that its returned value can change asynchronously.
 */
public interface Reader<T> extends AutoCloseable {
     ...
     * Multiple calls to this method without an intervening call to {@link #advance} should return the same result.
     *
     * @throws NoSuchElementException if the reader is at the beginning of the input and {@link #start} or
     * {@link #advance} wasn't called, or if the last {@link #start} or {@link #advance} returned {@code false}.
     */
     ...
     *
     * <p>
     * Usually, an implementation will simply return the immutable {@link Source} object from which the current
     * {@link Reader} was constructed, or delegate to the base class. However, when using or implementing this method on
     * a {@link BoundedReader}, special considerations apply, see documentation for
